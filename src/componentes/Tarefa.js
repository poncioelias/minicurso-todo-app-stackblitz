  ...
  return (
    <li key={tarefa.id} className="tarefa">
      ...
      <div className="tarefa__botoes">
        <button className="button" onClick={() => toggleConcluida(tarefa.id)}>{tarefa.completa ? 'Não concluída 🧐' : 'Feito ✅'}</button>
        <button className="button" onClick={() => toggleArquivada(tarefa.id)}>🔥</button>
      </div>
    </li>
  );
}
